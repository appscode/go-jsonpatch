...
func TestArrayBoolReplace(t *testing.T) {
	...
	change := patch[0]
	assert.Equal(t, "replace", change.Op, "they should be equal")
	...
	change = patch[1]
	assert.Equal(t, change.Op, "remove", "they should be equal")
...
func TestArrayAlmostSame(t *testing.T) {
	...
	change := patch[0]
	assert.Equal(t, "remove", change.Op, "they should be equal")
	assert.Equal(t, "/Lines/0", change.Path, "they should be equal")
	assert.Equal(t, nil, change.Value, "they should be equal")
	change = patch[1]
	assert.Equal(t, change.Op, "add", "they should be equal")
	assert.Equal(t, change.Path, "/Lines/9", "they should be equal")
